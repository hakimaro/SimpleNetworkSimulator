    ...
    const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    //std::cout << rbgSize << " " << m_cschedCellConfig.m_dlBandwidth << std::endl;
    ...
    for (itStats = m_flowStatsDl.begin(); itStats != m_flowStatsDl.end(); itStats++)
    {
        ...
        // Evolution, Ed Wiley)
        if ((*itStats).second.mod == 0)
        {
            if ((*itStats).second.flag == 0 || (*itStats).second.flag == 1 || (*itStats).second.flag == 2 || (*itStats).second.flag == 3)
            {
                std::cout << (*itStats).second.flag << " BEFORE lastAveragedThroughput = " << (*itStats).second.lastAveragedThroughput << " lastTtiBytesTrasmitted = " << (*itStats).second.lastTtiBytesTrasmitted << " " << (double)((*itStats).second.lastTtiBytesTrasmitted / 0.001) << std::endl;
                std::cout << ((1.0 - (1.0 / m_timeWindow)) * (*itStats).second.lastAveragedThroughput) << " " << ((1.0 / m_timeWindow) * (double)((*itStats).second.lastTtiBytesTrasmitted / 0.001)) << std::endl;
            }
            //учитываем в большинстве старую скорость
           ...
           ((1.0 / m_timeWindow) * (double)((*itStats).second.lastTtiBytesTrasmitted / 0.001));
            ...
            if ((*itStats).second.flag == 0 || (*itStats).second.flag == 1 || (*itStats).second.flag == 2 || (*itStats).second.flag == 3)
            {
                std::cout << "AFTER lastAveragedThroughput = " << (*itStats).second.lastAveragedThroughput << std::endl;
                (*itStats).second.flag++;
            }
        }
        if ((*itStats).second.mod == 1)
        {
            if ((*itStats).second.flag == 0 || (*itStats).second.flag == 1 || (*itStats).second.flag == 2 || (*itStats).second.flag == 3)
            {
                std::cout << (*itStats).second.flag << " BEFORE lastAveragedThroughput = " << (*itStats).second.lastAveragedThroughput << " lastTtiBytesTrasmitted = " << (*itStats).second.lastTtiBytesTrasmitted << " " << (double)((*itStats).second.lastTtiBytesTrasmitted / 0.001) << std::endl;
            ...
            (*itStats).second.instantThroughputDl.push_back((double)((*itStats).second.lastTtiBytesTrasmitted) / 0.001);
            double sum = std::accumulate((*itStats).second.instantThroughputDl.begin(), (*itStats).second.instantThroughputDl.end(), 0.0);
            (*itStats).second.lastAveragedThroughput = sum / (*itStats).second.instantThroughputDl.size();
        ...
        if ((*itStats).second.mod == 2)
        {
            if ((*itStats).second.flag == 0 || (*itStats).second.flag == 1 || (*itStats).second.flag == 2 || (*itStats).second.flag == 3)
            {
                std::cout << (*itStats).second.flag << " BEFORE lastAveragedThroughput = " << (*itStats).second.lastAveragedThroughput << " lastTtiBytesTrasmitted = " << (*itStats).second.lastTtiBytesTrasmitted << " " << (double)((*itStats).second.lastTtiBytesTrasmitted / 0.001) << std::endl;
            }
            //std::cout << (*itStats).first << std::endl;
            (*itStats).second.instantThroughputDl.push_back((double)((*itStats).second.lastTtiBytesTrasmitted) / 0.001);
            if ((*itStats).second.delta < m_timeWindow)
            {
                //std::cout << (*itStats).second.delta << std::endl;
                //mod 1
                double sum = std::accumulate((*itStats).second.instantThroughputDl.begin(), (*itStats).second.instantThroughputDl.end(), 0.0);
                (*itStats).second.lastAveragedThroughput = sum / (*itStats).second.instantThroughputDl.size();

                (*itStats).second.delta++;
            }
            else
            {
                double sum = std::accumulate((*itStats).second.instantThroughputDl.end() - m_timeWindow, (*itStats).second.instantThroughputDl.end(), 0.0);
                (*itStats).second.lastAveragedThroughput = sum / m_timeWindow;
            }
            if ((*itStats).second.flag == 0 || (*itStats).second.flag == 1 || (*itStats).second.flag == 2 || (*itStats).second.flag == 3)
            {
                std::cout << "AFTER lastAveragedThroughput = " << (*itStats).second.lastAveragedThroughput << std::endl;
    ...
    const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
    ...
    for (itStats = m_flowStatsUl.begin(); itStats != m_flowStatsUl.end(); itStats++)
    {
        ...
        (*itStats).second.lastAveragedThroughput =
            ((1.0 - (1.0 / m_timeWindow)) * (*itStats).second.lastAveragedThroughput) +
            ((1.0 / m_timeWindow) * (double)((*itStats).second.lastTtiBytesTrasmitted / 0.001));
        //(*itStats).second.instantThroughputUl.push_back((double)((*itStats).second.lastTtiBytesTrasmitted) / 0.001);
        //double sum = std::accumulate((*itStats).second.instantThroughputUl.begin(), (*itStats).second.instantThroughputUl.end(), 0.0);
        //(*itStats).second.lastAveragedThroughput = sum / (*itStats).second.instantThroughputUl.size();
        NS_LOG_INFO(this << " UE total bytes " << (*itStats).second.totalBytesTransmitted);
        NS_LOG_INFO(this << " UE average throughput " << (*itStats).second.lastAveragedThroughput);
        (*itStats).second.lastTtiBytesTrasmitted = 0;
    }
